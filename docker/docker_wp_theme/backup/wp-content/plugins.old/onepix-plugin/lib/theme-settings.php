...
define('ONEPIX_SHORTNAME', 'onepix'); // used to prefix the individual setting field id see onepixplugin_options_page_fields()  
...
function onepixplugin_get_settings() {  
      
    $output = array();  
      
    // put together the output array   
    $output['onepixplugin_option_name']       = 'onepixplugin_options'; // the option name as used in the get_option() call.   
    $output['onepixplugin_page_title']        = __( '1 Pixel Plugin Settings','onepixplugin_textdomain'); // the settings page title  
    $output['onepixplugin_page_sections']     = onepixplugin_options_page_sections(); // the setting section  
    $output['onepixplugin_page_fields']       = onepixplugin_options_page_fields();  
    $output['onepixplugin_contextual_help']   = onepixplugin_options_page_contextual_help();
...
*/  
function onepixplugin_create_settings_field( $args = array() ) {  
    ...
    $defaults = array(  
        ...
        'section' => 'main_section',                     // the section this setting belongs to — must match the array key of a section in onepixplugin_options_page_sections()  
    ...
    // additional arguments for use in form field output in the function onepixplugin_form_field_fn!  
    $field_args = array(  
        'type'      => $type,  
        'id'        => $id,  
        'desc'      => $desc,  
    ...
    add_settings_field( $id, $title, 'onepixplugin_form_field_fn', __FILE__, $section, $field_args );  
...
function onepixplugin_settings_page_fn() {  
// get the settings sections array  
    $settings_output = onepixplugin_get_settings();  
?>  
    <div class="wrap">  
        <div class="icon32" id="icon-options-general"></div>  
        <h2><?php echo $settings_output['onepixplugin_page_title']; ?></h2>  
          
        <form action="options.php" method="post">  
            <?php   
            // http://codex.wordpress.org/Function_Reference/settings_fields  
            settings_fields($settings_output['onepixplugin_option_name']);   

            // http://codex.wordpress.org/Function_Reference/do_settings_sections  
            do_settings_sections(__FILE__);   
            ?>  
            <p class="submit">  
                <input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes','onepixplugin_textdomain'); ?>" />  
            </p>  
...
function onepixplugin_register_settings(){  
      
    // get the settings sections array  
    $settings_output    = onepixplugin_get_settings();  
    $onepixplugin_option_name = $settings_output['onepixplugin_option_name'];  
      
    //setting  
    // register_setting( $option_group, $option_name, $sanitize_callback );  
    register_setting($onepixplugin_option_name, $onepixplugin_option_name, 'onepixplugin_validate_options' );  
      
    //sections  
    // add_settings_section( $id, $title, $callback, $page );  
    if(!empty($settings_output['onepixplugin_page_sections'])){  
        // call the "add_settings_section" for each!  
        foreach ( $settings_output['onepixplugin_page_sections'] as $id => $title ) {  
            add_settings_section( $id, $title, 'onepixplugin_section_fn', __FILE__);  
        }  
    }
    
    //fields  
    if(!empty($settings_output['onepixplugin_page_fields'])){  
        // call the "add_settings_field" for each!  
        foreach ($settings_output['onepixplugin_page_fields'] as $option) {  
            onepixplugin_create_settings_field($option);  
        }  
    }  
} 
...
function onepixplugin_validate_options($input) {
	...
	// collect only the values we expect and fill the new $valid_input array i.e. whitelist our option IDs
		...
		$options = $settings_output['onepixplugin_page_fields'];
					...
					switch ( $option['class'] ) {
						...
						case 'numeric':
							...
							if(is_numeric($input[$option['id']]) == FALSE) {
								add_settings_error(
									...
									__('Expecting a Numeric value! Please fix.','onepixplugin_textdomain'), // error message
						...
						case 'multinumeric':
							...
							if($input[$option['id']] !=''){
								...
								$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values','onepixplugin_textdomain');
							}else{
							...
							if($input[$option['id']] !='' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
								add_settings_error(
									...
									__('Expecting comma separated numeric values! Please fix.','onepixplugin_textdomain'), // error message
									'error' // type of message
								);
							}
						break;
						...
						case 'email':
							...
							$input[$option['id']] 		= trim($input[$option['id']]); // trim whitespace
							if($input[$option['id']] != ''){
								$valid_input[$option['id']] = (is_email($input[$option['id']])!== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!','onepixplugin_textdomain');
							}elseif($input[$option['id']] == ''){
								$valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.','onepixplugin_textdomain');
							}
							...
							if(is_email($input[$option['id']])== FALSE || $input[$option['id']] == '') {
								add_settings_error(
									...
									__('Please enter a valid email address.','onepixplugin_textdomain'), // error message
									'error' // type of message
								);
							}
						break;
...
function onepixplugin_show_msg($message, $msgclass = 'info') {  
 ...
 * ------------------------------------------------------------------------ */ 
 /*
 * @return calls onepixplugin_show_msg()
 */
function onepixplugin_admin_msgs() {
	
	// check for our settings page - need this in conditional further down
	$onepixplugin_settings_pg = strpos($_GET['page'], ONEPIX_PAGE_BASENAME);
	...
	if(current_user_can ('manage_options') && $onepixplugin_settings_pg !== FALSE && !empty($set_errors)){

		// have our settings succesfully been updated? 
		if($set_errors[0]['code'] == 'settings_updated' && isset($_GET['settings-updated'])){
			onepixplugin_show_msg("<p>" . $set_errors[0]['message'] . "</p>", 'updated');
		...
		// have errors been found?
			...
			foreach($set_errors as $set_error){
				...
				onepixplugin_show_msg("<p class='setting-error-message' title='" . $set_error['setting'] . "'>" . $set_error['message'] . "</p>", 'error');
			}
		}
	}
}
...
// admin messages hook!
add_action('admin_notices', 'onepixplugin_admin_msgs');
...
function  onepixplugin_section_fn($desc) {  
    //echo "<p>" . __('Settings for this section','onepixplugin_textdomain') . "</p>";
    echo "<p>" . __('','onepixplugin_textdomain') . "</p>"; 
}
...
function onepixplugin_form_field_fn($args = array()) {  
    ...
    $onepixplugin_option_name = $settings_output['onepixplugin_option_name'];  
    $options            = get_option($onepixplugin_option_name);  
    ...
    switch ( $type ) {
        ...
        case 'general':  
            ...
            echo "<input class='regular-text$field_class' type='text' id='$id' name='" . $onepixplugin_option_name . "[$id]' value='$options[$id]' />";  
        ...
        case 'img':
            $options[$id] = stripslashes($options[$id]);
            $options[$id] = esc_attr($options[$id]);
            echo "<input class='regular-text$field_class media-upload-input' type='text' id='$id' name='" . $onepixplugin_option_name . "[$id]' value='$options[$id]' />";
        ...
        case "multi-text":  
            foreach($choices as $item) {  
                ...
                $item[0] = esc_html__($item[0], 'onepixplugin_textdomain');  
                if (!empty($options[$id])) {  
                ...
                echo "<span>$item[0]:</span> <input class='$field_class' type='text' id='$id|$item[1]' name='" . $onepixplugin_option_name . "[$id|$item[1]]' value='$value' /><br/>";  
            }  
        ...
        case 'textarea':  
            ...
            echo "<textarea class='textarea$field_class' type='text' id='$id' name='" . $onepixplugin_option_name . "[$id]' rows='5' cols='50'>$options[$id]</textarea>";  
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";           
        break;  
          
        case 'select':  
            echo "<select id='$id' class='select$field_class' name='" . $onepixplugin_option_name . "[$id]'>";  
                foreach($choices as $item) {  
                    $value  = esc_attr($item, 'onepixplugin_textdomain');  
                    $item   = esc_html($item, 'onepixplugin_textdomain');  
        ...
        case 'select2':  
            ...
            foreach($choices as $item) {  
                ...
                $item = explode("|",$item);  
                $item[0] = esc_html($item[0], 'onepixplugin_textdomain');  
        ...
        case 'checkbox':  
            echo "<input class='checkbox$field_class' type='checkbox' id='$id' name='" . $onepixplugin_option_name . "[$id]' value='1' " . checked( $options[$id], 1, false ) . " />";  
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";  
        break;  
        ...
        case "multi-checkbox":  
            foreach($choices as $item) {  
                ...
                echo "<input class='checkbox$field_class' type='checkbox' id='$id|$item[1]' name='" . $onepixplugin_option_name . "[$id|$item[1]]' value='1' $checked /> $item[0] <br/>";  
            }  
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";  
        break;  
    }  
