...
define('CONTRIB_SHORTNAME', 'contrib'); // used to prefix the individual setting field id see contrib_options_page_fields()  
define('CONTRIB_PAGE_BASENAME', 'contrib-settings'); // the settings page slug  
...
function contrib_get_settings() {  
      
    $output = array();  
      
    // put together the output array   
    $output['contrib_option_name']       = 'contrib_options'; // the option name as used in the get_option() call.   
    $output['contrib_page_title']        = __( 'Contributors Settings','contrib_textdomain'); // the settings page title  
    $output['contrib_page_sections']     = contrib_options_page_sections(); // the setting section  
    $output['contrib_page_fields']       = contrib_options_page_fields();  
    $output['contrib_contextual_help']   = contrib_options_page_contextual_help();
...
*/  
function contrib_create_settings_field( $args = array() ) {  
    ...
    $defaults = array(  
        ...
        'section' => 'main_section',                     // the section this setting belongs to — must match the array key of a section in contrib_options_page_sections()  
    ...
    // additional arguments for use in form field output in the function contrib_form_field_fn!  
    $field_args = array(  
        'type'      => $type,  
        'id'        => $id,  
        'desc'      => $desc,  
    ...
    add_settings_field( $id, $title, 'contrib_form_field_fn', __FILE__, $section, $field_args ); 
...
function contrib_settings_page_fn() {  
// get the settings sections array  
    $settings_output = contrib_get_settings();  
?>  
    <div class="wrap">  
        <div class="icon32" id="icon-options-general"></div>  
        <h2><?php echo $settings_output['contrib_page_title']; ?></h2>  
          
        <form action="options.php" method="post">  
            <?php   
            // http://codex.wordpress.org/Function_Reference/settings_fields  
            settings_fields($settings_output['contrib_option_name']);   

            // http://codex.wordpress.org/Function_Reference/do_settings_sections  
            do_settings_sections(__FILE__);   
            ?>  
            <p class="submit">  
                <input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes','contrib_textdomain'); ?>" />  
            </p>  
...
function contrib_register_settings(){  
      
    // get the settings sections array  
    $settings_output    = contrib_get_settings();  
    $contrib_option_name = $settings_output['contrib_option_name'];  
      
    //setting  
    // register_setting( $option_group, $option_name, $sanitize_callback );  
    register_setting($contrib_option_name, $contrib_option_name, 'contrib_validate_options' );  
      
    //sections  
    // add_settings_section( $id, $title, $callback, $page );  
    if(!empty($settings_output['contrib_page_sections'])){  
        // call the "add_settings_section" for each!  
        foreach ( $settings_output['contrib_page_sections'] as $id => $title ) {  
            add_settings_section( $id, $title, 'contrib_section_fn', __FILE__);  
        }  
    }
    
    //fields  
    if(!empty($settings_output['contrib_page_fields'])){  
        // call the "add_settings_field" for each!  
        foreach ($settings_output['contrib_page_fields'] as $option) {  
            contrib_create_settings_field($option);  
        }  
    }  
} 
...
function contrib_validate_options($input) {
    ...
    $options = $settings_output['contrib_page_fields'];
                ...
                switch ($option['class']) {
                    ...
                    case 'numeric':
                        ...
                        if (is_numeric($input[$option['id']]) == FALSE) {
                            add_settings_error(
                                    ...
                                    CONTRIB_SHORTNAME . '_txt_numeric_error', // error ID
                                    __('Expecting a Numeric value! Please fix.', 'contrib_textdomain'), // error message
                    ...
                    case 'multinumeric':
                        ...
                        if ($input[$option['id']] != '') {
                            ...
                            $valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values', 'contrib_textdomain');
                        } else {
                        ...
                        if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
                            add_settings_error(
                                    $option['id'], // setting title
                                    CONTRIB_SHORTNAME . '_txt_multinumeric_error', // error ID
                                    __('Expecting comma separated numeric values! Please fix.', 'contrib_textdomain'), // error message
                                    'error' // type of message
                            );
                        }
                        break;
                    ...
                    case 'email':
                        ...
                        $input[$option['id']] = trim($input[$option['id']]); // trim whitespace
                        if ($input[$option['id']] != '') {
                            $valid_input[$option['id']] = (is_email($input[$option['id']]) !== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!', 'contrib_textdomain');
                        } elseif ($input[$option['id']] == '') {
                            $valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.', 'contrib_textdomain');
                        }
                        ...
                        if (is_email($input[$option['id']]) == FALSE || $input[$option['id']] == '') {
                            add_settings_error(
                                    $option['id'], // setting title
                                    CONTRIB_SHORTNAME . '_txt_email_error', // error ID
                                    __('Please enter a valid email address.', 'contrib_textdomain'), // error message
                                    'error' // type of message
                            );
                        }
                        break;
...
//							$valid_input[$option['id']] = addslashes($input[$option['id']]); ben commented this out because it was adding extra slashes
            ...
            case 'sort':
                ...
                //if empty fill user draggable with default value
                if(!$input[$option['id']]) {
...
//                        
//                        $numItems = count($user_query->results);
//                        $i = 0;
...
//                                    $comma = "";
//                                }
//                                $valid_input[$option['id']] .= (string)($author->ID . $comma);
//                            }
//                        }
                    
                
                //else validate to make sure it's numeric
                } else  {
...
function contrib_show_msg($message, $msgclass = 'info') {  
 ...
 * ------------------------------------------------------------------------ */ 
 /*
 * @return calls contrib_show_msg()
 */
function contrib_admin_msgs() {
	
        $contrib_settings_pg = FALSE;
	// check for our settings page - need this in conditional further down
        if(isset($_GET['page'])) {
            $contrib_settings_pg = strpos($_GET['page'], CONTRIB_PAGE_BASENAME);
        }
	...
	if(current_user_can ('manage_options') && $contrib_settings_pg !== FALSE && !empty($set_errors)){

		// have our settings succesfully been updated? 
		if($set_errors[0]['code'] == 'settings_updated' && isset($_GET['settings-updated'])){
			contrib_show_msg("<p>" . $set_errors[0]['message'] . "</p>", 'updated');
		...
		// have errors been found?
			...
			foreach($set_errors as $set_error){
				...
				contrib_show_msg("<p class='setting-error-message' title='" . $set_error['setting'] . "'>" . $set_error['message'] . "</p>", 'error');
			}
		}
	}
}
...
// admin messages hook!
add_action('admin_notices', 'contrib_admin_msgs');
...
function  contrib_section_fn($desc) {  
    //echo "<p>" . __('Settings for this section','contrib_textdomain') . "</p>";
    echo "<p>" . __('','contrib_textdomain') . "</p>"; 
}
...
function contrib_form_field_fn($args = array()) {  
    ...
    $contrib_option_name = $settings_output['contrib_option_name'];  
    $options            = get_option($contrib_option_name);  
    ...
    switch ( $type ) {
        ...
        case 'general':  
            ...
            echo "<input class='regular-text$field_class' type='text' id='$id' name='" . $contrib_option_name . "[$id]' value='$options[$id]' />";  
        ...
        case "multi-text":  
            foreach($choices as $item) {  
                ...
                $item[0] = esc_html__($item[0], 'contrib_textdomain');  
                if (!empty($options[$id])) {  
                ...
                echo "<span>$item[0]:</span> <input class='$field_class' type='text' id='$id|$item[1]' name='" . $contrib_option_name . "[$id|$item[1]]' value='$value' /><br/>";  
            }  
        ...
        case 'textarea':  
            ...
            echo "<textarea class='textarea$field_class' type='text' id='$id' name='" . $contrib_option_name . "[$id]' rows='5' cols='50'>$options[$id]</textarea>";  
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";           
        break;  
          
        case 'select':  
            echo "<select id='$id' class='select$field_class' name='" . $contrib_option_name . "[$id]'>";  
                foreach($choices as $item) {  
                    $value  = esc_attr($item, 'contrib_textdomain');  
                    $item   = esc_html($item, 'contrib_textdomain');  
        ...
        case 'select2':  
            ...
            foreach($choices as $item) {  
                ...
                $item = explode("|",$item);  
                $item[0] = esc_html($item[0], 'contrib_textdomain');  
        ...
        case 'checkbox':  
            echo "<input class='checkbox$field_class' type='checkbox' id='$id' name='" . $contrib_option_name . "[$id]' value='1' " . checked( $options[$id], 1, false ) . " />";  
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";  
        break;  
        ...
        case "multi-checkbox":  
            foreach($choices as $item) {  
                ...
                echo "<input class='checkbox$field_class' type='checkbox' id='$id|$item[1]' name='" . $contrib_option_name . "[$id|$item[1]]' value='1' $checked /> $item[0] <br/>";  
            }  
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";  
        break;
        ...
        case "sort":
            $options[$id] = stripslashes($options[$id]);  
            $options[$id] = esc_attr( $options[$id]);  
            echo "<input class='sort$field_class' type='text' id='$id' name='" . $contrib_option_name . "[$id]' value='$options[$id]' />";  
            //echo ($desc != '') ? "<br /><span class='description'>$desc</span><br/><br /><br />" : "";
            echo "<span class='description-sort'>Drag from left to right. Re-arrange the right column for display order</span>";            
            //render our sorter (in sort-order.php)
            render_sort_order();
